...
extern int xcpu_execute( xcpu *c ) {
  ...
  assert( c );
//fprintf(stdout,"r1:%d\t, r2:%d\t, op:%4.4x\t, pc:%d\t\n",r1,r2,op, c->pc);
  ...
  case I_OUT:
    ...
    break;
  case I_CLI:
    c->state= c->state&0XFFFB;
    break;
  case I_STI:
   c->state = c->state|0x0004;
   break;
  case I_IRET:
   c->pc  = pop(c);
   c->state = pop(c);
...
extern int xcpu_exception( xcpu *c, unsigned int ex ) {
    ...
    push(c, c->pc);
    c->state = c->state | 0x0004;
    c->pc = (c->memory[MEM_IDX(c->itr + index )] )<< 8 | (c->memory[MEM_IDX(c->itr + index + 1)] );
    // fprintf(stdout, "c->itr :%d\n", c->itr );
    // int i;
    // for( i = 0 ; i < 100 ; i++){
    //   fprintf(stdout, "c->memory[ %d] = %d \n", i, c->memory[MEM_IDX( i)] );
    // }
    // fprintf(stdout,"c->pc value is %d (c->itr+index )\n if not c->itr+index : %d\n ", c->pc, (c->memory[MEM_IDX(c->itr + index )] << 8) );
